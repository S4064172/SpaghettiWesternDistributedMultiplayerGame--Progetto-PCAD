...
//--controllare i lock su mapInfo
	...
	public Server (int count)
	{
 		...
 		try {
			...
			System.err.println("Server gia online!!!O c'e un servizio attivo sulla porta 9000 o 9001 0 9002 " + e);
			return;
		}
	...
	private ErrorCodeEnum Registrazione(Socket clientSocket,ObjectInputStream inStreamRegistrazione, ObjectOutputStream outStreamRegistrazione )
	{
		...
		try{
			...
			System.err.println("-----------------LetturaDati-------------------");
			...
			System.err.println("-----------------FineLetturaDati-------------------");
			
		} catch (ClassNotFoundException | IOException e){
			System.err.println("Errore Lettura Registrazione!!! Registrazione Finita!! "+e);
			try {
				...
				System.err.println("Errore notifica errore lettura!! "+e1);
			}
		...
		{	
			...
			try {
				...
				System.err.println("NickName già presente");
			} catch (IOException e1) {
				//se non riesco a notificare al client l'errore, che il time out 
				//del client e del server che fa resettare la comunicazione
				System.err.println(errorCode.get(keyErrorCode.get(ErroreUsernameGiaPresente.ordinal())) +" "+ e1);
			}
			return  ErroreRegistrazioneRestart;
		}	
		//Se disponibile
		if(mapGiocatori.size()>=MaxGiocatori)
		{
			LockMapGiocatori.unlock();
			try {
				System.err.println(keyErrorCode.get(ServerPieno.ordinal()));
				outStreamRegistrazione.writeObject(keyErrorCode.get(ServerPieno.ordinal()));		
			} catch (IOException e) {
				System.err.println("Errore segnalazione server pieno "+e);
			}
		...
		if ( (inputStreamFazione.equals(errorCode.get(keyErrorCode.get(FazioneCattivi.ordinal()))) &&  cattivi.size() > buoni.size() ) || (inputStreamFazione.equals(errorCode.get(keyErrorCode.get(FazioneBuoni.ordinal()))) && cattivi.size() < buoni.size() ) )
		{
			...
			try {
				...
				System.err.println(errorCode.get(keyErrorCode.get(ErroreFaziniSquilibrate.ordinal())));
			} catch (IOException e) {
				System.err.println("Errore segnalazione errore fazione squilibrate "+e);
			}
			return  ErroreRegistrazioneRestart;
		}	
...
//		System.err.println("server reciver user***"+inputStreamUserName);
//		System.err.println("server reciver pass ***"+inputStreamPassword);
//		System.err.println("server reciver fazine***"+inputStreamFazione);
//		System.err.println("server reciver porta***"+inputStreamPorta);
//		System.err.println("server reciver porta chat***"+inputStreamPortaChat);
//		System.err.println("server reciver porta pingPogn***"+inputSteamPortaPingPong);
//		System.err.println("server reciver ip***"+String.valueOf(clientSocket.getInetAddress()).substring(1));
...
//		System.err.println("Port: "+mapGiocatori.get(inputStreamUserName).getPort());
//		System.err.println("Port chat: "+mapGiocatori.get(inputStreamUserName).getPortaChat());
//		System.err.println("Port pingPong: "+mapGiocatori.get(inputStreamUserName).getPortaPingPong());
//		System.err.println("indirizzo ip: "+mapGiocatori.get(inputStreamUserName).getIp());
//		System.err.println("user: "+mapGiocatori.get(inputStreamUserName).getUsername());
//		System.err.println("fazione: "+mapGiocatori.get(inputStreamUserName).getFazione());
//		System.err.println("pw: "+mapGiocatori.get(inputStreamUserName).getPassword());
		...
		try{
			...
			System.err.println("Errore notifica avvenuta registrazione!!! "+e1);
		}
	...
	private ErrorCodeEnum Riconnessione(Socket clientSocket,ObjectInputStream inRiconnessione, ObjectOutputStream outRiconnessione )
	{
		...
		try{
			...
			System.err.println(errorCode.get(keyErrorCode.get(ErroreLetturaDatiDiRegistrazione.ordinal()))+" "+e);
			try {
				outRiconnessione.writeObject(keyErrorCode.get(ErroreLetturaDatiDiRegistrazione.ordinal()));
			} catch (IOException e1) {
				//se mi salta anche questa notifica, c'è il timeOut nel client che fa resettare tutto
				System.err.println("Errore notifica errore Riconnessione!! "+e1);
			}
			return ErroreRegistrazioneRestart;
		}
		/*
...
//		System.err.println("U----->"+inputStreamUserName);
//		System.err.println("PWR----->"+ mapGiocatori.get(inputStreamUserName).getPassword());
//		System.err.println("PW----->"+inputStreamPassWord);
//		System.err.println("----->"+mapGiocatori.containsKey(inputStreamUserName));
		if ( mapGiocatori.containsKey(inputStreamUserName)	&& 
			 mapGiocatori.get(inputStreamUserName).getPassword().equals(inputStreamPassWord))											  
		{
			 ...
			 */
			System.err.println("----------ControlloOffLineGiocatore---------------");
			System.err.println(mapGiocatori.get(inputStreamUserName).getIp()+"----"+ mapGiocatori.get(inputStreamUserName).getPortaPingPong());
			try(Socket temp = new Socket(mapGiocatori.get(inputStreamUserName).getIp(), mapGiocatori.get(inputStreamUserName).getPortaPingPong())) 
			{
				...
				System.err.println("----------AttesaRisposta---------------");
				try(ObjectInputStream inStreamRicconnesione = new ObjectInputStream(temp.getInputStream()))
				{
					...
					if(cod.equals(keyErrorCode.get(ClientOnline.ordinal())))
					{
						...
						System.err.println("----------FineControlloOffLineGiocatore---------------");
						try {
							outRiconnessione.writeObject(keyErrorCode.get(ErroreClientOnline.ordinal()));
							return ErroreFineRegistrazione;
						} catch (IOException e) {
							System.err.println("Errore segnalazione errore per ClientOnline "+ e);
							return ErroreFineRegistrazione;
						}
					}
					return ErroreFineRegistrazione;
				...
				System.err.println("Client scelto offline " + e2);
			}
			System.err.println("----------FineControlloOffLineGiocatore---------------");
			LockMapGiocatoriInfo.unlock();
			LockMapGiocatori.unlock();	
			try {
				...
				//se mi salta anche questa notifica, c'è il timeOut nel client che fa resettare tutto
				System.err.println("errore notifica avvenuta riconnessione "+e1);
			}
			System.err.println(errorCode.get(keyErrorCode.get(ConfermaAvvenutaRiconnessione.ordinal())));
			/*
...
System.err.println("--->Attesa permesso riconnessione...");
LockGame.lock();
System.err.println("--->Permesso riconnessione ricevuto...");
LockMapGiocatoriInfo.lock();
LockMapGiocatori.lock();	
...
System.err.println("--->Fine riconnessine....");
LockGame.unlock();
System.err.println("--->Scancio della risorza....");
		...
		else
		{
			...
			try {
				...
				System.err.println(errorCode.get(keyErrorCode.get(ErroreRiconnessione.ordinal())));
			} catch (IOException e1) {
				//se mi salta anche questa notifica, c'è il timeOut nel client che fa resettare tutto
				System.err.println("Errore segnalazione errore riconnessione "+e1);
			}
			return ErroreRegistrazioneRestart;
		}	
		System.err.println("---------------Fine Riconnessione---------------------");
			...
			public  ErrorCodeEnum call()
			{
				...
				try (ObjectOutputStream outStream = new ObjectOutputStream(clientSocket.getOutputStream()))
				{
					try 
					{
						...
						//se non riesco a notificare al client l'errore, che il time out 
						//del client e del server che fa resettare la comunicazione
						System.err.println("Errore scittura primo messaggio!! "+e1);
						//e1.printStackTrace();
						return ErroreFineRegistrazione;
					}
					...
					try (ObjectInputStream inputStream = new ObjectInputStream(clientSocket.getInputStream()))
					{
						do
						{
							try {
								...
								System.err.println("errore set time connessione!!! "+e);
								messError=ErroreRegistrazioneRestart;					
							}
							
							System.err.println("------Primo messaggio Connessione o Riconnessione?------");
							try	{
								frtMessage = (String)inputStream.readObject();
							}catch (SocketTimeoutException e){
								System.err.println("Tempo scaduto "+e);
								try {
									//System.err.println(errorCode.get(keyErrorCode.get(ConfermaTempoRegistrazioneScaduto.ordinal())));
									outStream.writeObject(keyErrorCode.get(ConfermaTempoRegistrazioneScaduto.ordinal()));
								} catch (IOException e1) {
									//se non riesco a notificare al client l'errore, che il time out 
									//del client e del server che fa resettare la comunicazione
									System.err.println("Errore segnalazione Tempo scaduto!! " + e1);
									//e1.printStackTrace();
								}
...
//							System.err.println("------------------------------------------------"+frtMessage);
						...
						System.err.println("Errore apertuta stream in!! " +e1);
						e1.printStackTrace();
						messError = ErroreFineRegistrazione;
					}						
				} catch (IOException e1) {
					System.err.println("Errore apertuta stream out!! "+e1);
					messError=ErroreFineRegistrazione;
				}
				try {
					clientSocket.close();
				} catch (IOException e) {
					System.err.println("Errore chiusura socket e buffer!!!" + e);
				}
				return messError;	
			}
		};
	...
	private void FasePreliminare() 
	{
		...
		while(!IsStop)
		{
			...
			try
			{
				...
				System.err.println("Tempo di connessione scaduto; possibilile solo la riconnessione!! " + e);
			} catch (IOException e) {
				System.err.println("chiusura socket connessione!!" + e);
				//e.printStackTrace();
			}
		}
	}
...
//						System.err.println("--------------------------------------------2----------------------------------------------");							
						...
						System.err.println("SEI TRUE !!!!!!");
//					//con le stampe di debug bisogna campiare i tempi
//					 for (String string : mapGiocatoriInfo.keySet()) 
//					 {
//						System.err.println("-----------------------\n-->"+mapGiocatoriInfo.get(string).getPos());
//						System.err.println("-->"+mapGiocatoriInfo.get(string).getPunteggio());
//					 }
//						for(int index=0; index < infoGraph.getNVertex(); index++)
//						{
//							System.err.println("--->"+grafo.getListNomiCitta().get(index)+posGraph.getListVertex().get(index).getCountBuoni());
//							System.err.println("--->"+grafo.getListNomiCitta().get(index)+posGraph.getListVertex().get(index).getCountCattivi());
//						}
						...
						try(ObjectInputStream inStream = new ObjectInputStream(socketAggiornamento.getInputStream()))
						{
							...
							System.err.println("nome giocatore : "+player.getUsername() );
							try{
								...
								System.err.println("SEI false !!!!!!");
								}
							}catch(SocketTimeoutException | ClassNotFoundException e){
								System.err.println("fine tempo di attesa conferma aggiornamento " + e);
								//e.printStackTrace();
							}
							outStream.writeObject(mapGiocatoriInfo.get(username).IsUpdate());
						}catch(IOException  e){
							System.err.println("Errore apertura buffer in " + e);
							//e.printStackTrace();
						}
					}catch(IOException e){
						System.err.println("Errore apertura buffer out " + e);
						//e.printStackTrace();
					}
				} catch (IOException e) {
					System.err.println("Errore Fase aggiornamento client " + e);
					//e.printStackTrace();
				} 
//				System.err.println("------------------------------------------3------------------------------------------------");
	...
	private void Inizializzazione()
	{
		...
		System.err.println("tutti i task sono stati caricati");
		...
		try {
			...
			System.err.println("Inizializzazione fallita per qualche client "+e);
			System.err.println("Chiusura forzata del pool di aggiornamento");
			poolAggiornamento.shutdownNow();	
		}
		System.err.println("tuttu i task sono stati eseguiti");
	}
	...
	private  Callable<ErrorCodeEnum> AggiornamentoComando(final String username,final String comando, final String usernamePlayer) 
	{
		Callable<ErrorCodeEnum> task = new Callable<ErrorCodeEnum>() {
			...
			public  ErrorCodeEnum call()
			{
				System.err.println("Inizio fase aggirnamento comando");
				...
				if ( !palyerBackGround.getUsername().equals(usernamePlayer))
				{
						...
						ObjectOutputStream	outStreamBackGroud = new ObjectOutputStream(aggiornamentiPartira.getOutputStream()))
					{
						...
						System.err.println("Errore comunicazine client per aggiornamento " +e);
					}
					System.err.println("Fine fase aggirnamento comando");
				}
				return null;
			}
		};
	...
	{
		System.err.println("--->Permesso Riconnessione");
		LockGame.unlock();
		try {
		...
		System.err.println("--->Fine Permesso Riconnessione");
		LockGame.lock();
		boolean flag=true;
		...
		System.err.println("--->Ritorno al gioco");
	}
	...
	private void propagazioneInfo(ConcurrentLinkedQueue<String> listGiocatori, String comando,String user)
	{
		...
		for (String username : listGiocatori) {
			...
			{
				System.err.println("--->"+username+mapGiocatori.get(username).getPorta());
				poolAggiornamento.submit(AggiornamentoComando(username, comando,user)); 
			}else
				System.err.println("-->user"+username+"false");
			LockListBuoniCattivi.lock();
		}								
		LockListBuoniCattivi.unlock();
	}
...
LockGame.unlock();
			...
			{//turno cattivi...
				if (cattivi.size()>0)
				{//se c'e un cativo...
					System.err.println("Turno fazione"+ errorCode.get(keyErrorCode.get(FazioneCattivi.ordinal())));
			...
			{//turno buoni...
				if (buoni.size()>0)
				{//se c'è un buono...
					System.err.println("Turno fazione"+ errorCode.get(keyErrorCode.get(FazioneBuoni.ordinal())));
...
//}
			...
			if (player!=null)
			{
				System.err.println("---->"+player.getUsername()+"--"+mapGiocatoriInfo.get(player.getUsername()).IsUpdate());
				
				LockMapGiocatori.lock();
				System.err.println("------------TENTATIVO---------"+player.getUsername());
				try (Socket socketPartita = new Socket(player.getIp(), player.getPorta()))
				{
					System.err.println("client online");
					LockMapGiocatori.unlock();
						
					try (ObjectOutputStream outStream = new ObjectOutputStream(socketPartita.getOutputStream()))
					{
						try {
							System.err.println("------------INIZIOTURNO---------"+player.getUsername());
							outStream.flush();
							...
							System.err.println("Errore segnalazione turno " +e1);
							e1.printStackTrace();
						}
...
//}
									...
									System.err.println("ComandoRicevuto"+comando);
									...
									try {
										...
										System.err.println("Errore notifica mossa " +e);
									}
									
									if (temp[0].equals((keyErrorCode.get(ComandoMossa.ordinal()))))
									{
										System.err.println("Il Giocatore Si è Mosso, ed ha :"+mapGiocatoriInfo.get(player.getUsername()).getPunteggio());
										...
										{
											System.err.println("Munizioni prese :"+ricarica);
											if(IsBrutto)
											{
...
//}
										...
										{
											System.err.println("Il Giocatore Attacca: "+player.getUsername()+" attacca "+temp[1]);
													...
													while(!IsSfida)
													{
														...
														try{
															...
															try{
																...
																System.err.println("ComandoSfidante ---------------> "+comandoSfidante);
																System.err.println("ComandoAttaccante -------------> "+comandoAttaccante);
																...
																{																
																	System.err.println("(PrimoIf)"+keyErrorCode.get(FineSfida.ordinal()));
																	...
																	System.err.println("---->Risultato Sfida ---->>"+munizioniAttaccante+"|----|"+munizioniSfidante+"<<----");
																...
																if (comandoSfidante.equals(keyErrorCode.get(HaiPerso.ordinal())) && !comandoAttaccante.equals(keyErrorCode.get(HaiPerso.ordinal())))
																{														
																	System.err.println("(SecondoIf)"+keyErrorCode.get(FineSfida.ordinal()));
																...
																if (!comandoSfidante.equals(keyErrorCode.get(HaiPerso.ordinal())) && comandoAttaccante.equals(keyErrorCode.get(HaiPerso.ordinal())))
																{
																	System.err.println("(TerzoIf)"+keyErrorCode.get(FineSfida.ordinal()));
																...
																{
																	System.err.println("(QuartoIf)"+keyErrorCode.get(FineSfida.ordinal()));
																...
																{
																	System.err.println("(QuintoIf)"+keyErrorCode.get(FineSfida.ordinal()));
															...
															{
																System.err.println("(Cathc1)"+keyErrorCode.get(FineSfida.ordinal())+" "+e2);
																outStream.writeObject(keyErrorCode.get(HaiVinto.ordinal()));
																outStreamSfidante.writeObject(keyErrorCode.get(HaiPerso.ordinal()));
														...
														} catch (ClassNotFoundException | IOException e2)
														{
															...
															{
																comandoSfidante=(String) inStreamSfidante.readObject();
																System.err.println("(Cathc2)"+keyErrorCode.get(FineSfida.ordinal())+" "+e2);
																outStream.writeObject(keyErrorCode.get(HaiPerso.ordinal()));
																outStreamSfidante.writeObject(keyErrorCode.get(HaiVinto.ordinal()));
															...
															{
																System.err.println("(Catch3)"+keyErrorCode.get(FineSfida.ordinal()));
																outStream.writeObject(keyErrorCode.get(PareggioFineMosse.ordinal()));
																outStreamSfidante.writeObject(keyErrorCode.get(PareggioFineMosse.ordinal()));
																System.err.println("---->Risultato Sfida ---->>"+munizioniAttaccante+"|----|"+munizioniSfidante+"<<----");
																outStream.writeObject(keyErrorCode.get(FineSfida.ordinal()));
...
//}
	...
	//sbagli a fare i conti delle munizioni							
								...
								System.err.println("Errore time out lettura "+e1);
							} catch (ClassNotFoundException | IOException e2) {
								System.err.println("errore lettura "+e2);
							} 
							try {
								
								outStream.writeObject(keyErrorCode.get(ConfermaFineTruno.ordinal()));
							} catch (IOException e1) {
								System.err.println("Errore notifica fine turno "+e1);
							}
concessaRiconnessione(100, 1100);
//for (String string : mapGiocatoriInfo.keySet()) 
//{
...
//}
							...
							System.err.println("Errore "+e1);
						}	
					}catch (IOException e1) {
						System.err.println("Errore apetura stream "+e1);
					}					
				...
				} catch (IOException e) 
				{
					 ...
					 */
					System.err.println("--->Client "+player.getUsername()+" risulta offLine");
					LockMapGiocatoriInfo.lock();
...
//}
					...
					System.err.println("Client offline :"+player.getUsername());
					
				}finally 
				{
					System.err.println("---------------SpostamentoBrutto------------------");
					System.err.println("VecchiaPosizioneBrutto"+posizione);
					...
					System.err.println("NuovaPosizioneBrutto"+posizione);				
					System.err.println("----------------FineSpostamentoBrutto-----------------");
				}
			}
		}
...
LockGame.unlock();
	...
	private  Callable<ErrorCodeEnum> AggiornamentoChat(String username, String text) 
	{
		Callable<ErrorCodeEnum> task = new Callable<ErrorCodeEnum>() {
			...
			public  ErrorCodeEnum call()
			{
				System.err.println("-------------InizioFaseAggirnamentoChat-------------");
				
				LockMapGiocatori.lock();
				Giocatore palyerBackGround = mapGiocatori.get(username);
				LockMapGiocatori.unlock();
				
//					System.err.println(palyerBackGround.getIp()+palyerBackGround.getPort());
					...
					System.err.println("------------FineFaseAggirnamentoComando------------");			
				return null;
			}
		};
		return task;
...
//							System.err.println("--------"+username);
					...
					System.err.println("Errore messaggi chat " + e);
				}
			}catch (SocketTimeoutException e) {
				System.err.println("Errore messaggi chat timeOut " + e);
			} catch (IOException e) {
				System.err.println("Errore messaggi chat " + e);
			}
		}
	...
	public void start() 
	{
		...
		System.err.println("Attesa Inzio Gioco");
		try {
		...
		LockMapGiocatoriInfo.unlock();
		System.err.println("Inizializzazione gioco");
		Inizializzazione();
		System.err.println("Fine Inizializzazione gioco");
		System.err.println("Inizio gioco");
		gioco();
	...
	class TimeOut extends TimerTask
	{
		...
		public synchronized void run() 
		{
			...
			try {
				...
				// TODO Auto-generated catch block
				System.err.println("Forzatura chiusura poolAggiornamento "+e);
				poolAggiornamento.shutdownNow();
				//e.printStackTrace();
			}
			try {
				...
				// TODO Auto-generated catch block
				System.err.println("Forzatura chiusura poolResgistrazione "+e);
			...
			try {
				...
				// TODO Auto-generated catch block
				System.err.println("Forzatura chiusura poolChat " +e);
			...
			if(threadPingPong.isAlive())
				...
				{
					System.err.println("-->Chiusura ThreadPingPong");
				} catch (IOException e) {
					System.err.println("-->Chiusura ThreadPingPong " +e);
					//e1.printStackTrace();
				}
		...
		class TimerRegistrazioene extends Thread
		{
			...
			public void run() {
				int i=0;
				System.err.println("TimeOutIscrizione*******************************");
			while (i < NControlli)
			{			try {
					Thread.sleep(TempoIscrizione/NControlli);
				} catch (InterruptedException e) {
				System.err.println("errore sleep timerRegistrazione");
				}
				pingPong.signal();
				System.err.println("-->controllo" + i);
				LockMapGiocatori.lock();
				System.err.println("-->Giocatori presenti -- Giocatori necessari --->"+mapGiocatori.size()+" -- "+MaxGiocatori);
				if (mapGiocatori.size()>=MaxGiocatori)
				{
					LockMapGiocatori.unlock();
					break;
				...
				if (i >= NControlli && mapGiocatori.size()<MinGiocatori)
				{
					...
					System.err.println("Giocatori insufficenti");
					i=0;
				}
				LockMapGiocatori.unlock();
			}
			...
			pingPong.signal();
			lockInizioGame.unlock();
			System.err.println("Fine TimeOut iscrizione*******************************");
			}	
		}
		...
		class PingPong extends Thread
		{
			...
			public void run() 
			{
				...
				{
					lockInizioGame.unlock();
					System.err.println("TimeOut pingPong*******************************");
					lockPingPong.lock();
					try {
						condPingPong.await();
					} catch (InterruptedException e1) {
...
//						System.err.println(mapGiocatori.get(username).getIp() +" --- "+ mapGiocatori.get(username).getPortaPingPong());
							...
							ObjectInputStream inStreamPingPong = new ObjectInputStream(pingPong.getInputStream())	)
						{
							...
							if(cod.equals(keyErrorCode.get(ClientOnline.ordinal())))
								System.err.println("-->client online "+username);
							else
							{
								System.err.println("-->client offline "+username);
							...
							e.printStackTrace();
							System.err.println("client offline "+username+" -- "+e);
							mapGiocatori.remove(username);
							mapGiocatoriInfo.remove(username);
							buoni.remove(username);
						}
...
//						System.err.println(mapGiocatori.get(username).getIp() +" --- "+ mapGiocatori.get(username).getPortaPingPong());
					...
					LockListBuoniCattivi.unlock();
					System.err.println("FineTimeOutPingPong*******************************");
					lockInizioGame.lock();
				}
				lockInizioGame.unlock();
			}	
		...
		class Munizioni extends Thread
		{
			...
			public  void  run()
			{
				synchronized(this)
				{
					while (!IsStop)
					{
							...
							if(count < max)
							{
								...
								{
									System.err.println("--->munizioni sull citta "+grafo.cityAtIndex(num));
				...
				for (ThreadMunizioni threadMuni : listMunizioni)
					try {
						...
						System.err.println("Errore attesa join Munizioni");
					}
			...
			class ThreadMunizioni extends Thread
			{
				...
				public  void run() {
					...
					try {
						System.err.println("/*/*/*/*/*TIMEOUT MUNIZIONI/*/*/*/*/*/*/*/"+numMunizioni);
						lockMun.lock();
						...
						lockMun.unlock();
						System.err.println("/*/*/*/*/*FINE TIMEOUT MUNIZIONI/*/*/*/*/*/*/*/");
						count--;
